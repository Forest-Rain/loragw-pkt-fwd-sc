...
#ifdef LORAGW_PKT_FWD_USING_DEBUG_JIT_QUEUE
    #define MSG(args...)                    LOG_I(args)
    #define MSG_DEBUG(args...)              LOG_D(args)
#else
    #define MSG(args...)
...
enum jit_error_e jit_enqueue(struct jit_queue_s *queue, uint32_t time_us, struct lgw_pkt_tx_s *packet, enum jit_pkt_type_e pkt_type) {
    ...
    MSG_DEBUG("jit_enqueue: Current concentrator time is %u, pkt_type=%d\n", time_us, pkt_type);
